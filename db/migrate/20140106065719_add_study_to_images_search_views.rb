class AddStudyToImagesSearchViews < ActiveRecord::Migration[4.2]
  def up
    execute <<-SQL
    ...
    if ActiveRecord::Base.connection.adapter_name == 'PostgreSQL'
      ...
      execute <<-SQL
        create view studies_search as select studies.id as study_id, studies.name as text, 'study_' || studies.id as result_id, 'study' as result_type from studies;
      SQL
      execute <<-SQL
        create view centers_search as select centers.study_id as study_id, centers.code || ' - ' || centers.name as text, 'center_' || centers.id as result_id, 'center' as result_type from centers;
      SQL
      execute <<-SQL
        create view patients_search as select centers.study_id as study_id, centers.code || patients.subject_id as text, 'patient_' || patients.id as result_id, 'patient' as result_type from patients, centers where centers.id = patients.center_id;
      SQL
      execute <<-SQL
