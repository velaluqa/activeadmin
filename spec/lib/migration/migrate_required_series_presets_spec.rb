        ...
        expect(Version.where(item_type: 'RequiredSeries').map(&:attributes))
          .to include(include(
                ...
                'object_changes' => include(
                  'name' => [nil, 'SPECT_1'],
                  'visit_id' => [nil, visit.id],
                  'created_at' => [nil, version1.created_at.iso8601],
                  'updated_at' => [nil, version1.created_at.iso8601]
                ),
        ...
        expect(Version.where(item_type: 'RequiredSeries').map(&:attributes))
          .to include(include(
                ...
                'object_changes' => include(
                  'name' => [nil, 'SPECT_3'],
                  'visit_id' => [nil, visit.id],
                  'created_at' => [nil, version2.created_at.iso8601],
                  'updated_at' => [nil, version2.created_at.iso8601]
                ),
