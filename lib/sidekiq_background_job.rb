...
module SidekiqBackgroundJob
  ...
  included do
    ...
    def perform(job_id, versions_metadata, *args)
      @progress_total = 1
      @progress = 0
      ...
      ::PaperTrail.request.controller_info = versions_metadata.symbolize_keys.merge(
    ...
    class << self
      ...
      def perform_async(*args)
        ...
        background_job = BackgroundJob.create(
          { user_id: PaperTrail.request.whodunnit }.merge(options)
        )

        versions_metadata = PaperTrail.request.controller_info || {}
        perform_async_without_job(background_job.id, versions_metadata, *args)
