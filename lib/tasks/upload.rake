...
namespace :erica do
  ...
  task :image_series_report, [:year, :month, :day] => [:environment] do |t,args|
    if(args[:year].blank? or args[:month].blank? or args[:day].blank?)
      $stderr.puts 'No date given, aborting.'
      next
    end

    start_time = DateTime.new(args[:year].to_i, args[:month].to_i, args[:day].to_i)
    $stderr.puts "Compiling report of all image series uploaded since #{start_time.to_s}..."

    image_series = ImageSeries.where('created_at >= ?', start_time)
    $stderr.puts "Found #{image_series.count} created since #{start_time.to_s}..."
    
    puts '"Study Number","ERICA Subject ID","DICOM Patient Name","ERICA DoI","DICOM DoI","ERICA Series Name","Images Count","URL","Patient Mismatch"'
    ...
    image_series.find_each do |is|
      ...
      puts '"'+study_name+'","'+erica_subject_id+'","'+dicom_patient_name.to_s+'","'+erica_imaging_date.to_s+'","'+dicom_imaging_date.to_s+'","'+series_description+'","'+images_count.to_s+'","http://192.168.1.28/admin/image_series/'+is.id.to_s+'",'+(mismatch ? 'Yes' : 'No')
