    ...
    class IndexAsTable
      ...
      def customizable_default_actions(ability = nil, &block)
        column '' do |resource|
          ...
          if controller.action_methods.include?('edit') and not except.include?(:edit)
            links << link_to(I18n.t('active_admin.edit'), edit_resource_path(resource), :class => "member_link edit_link")
          end
          if controller.action_methods.include?('destroy')  and not except.include?(:destroy)
