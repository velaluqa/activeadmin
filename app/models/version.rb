...
class Version < PaperTrail::Version
  ...
  scope :for_center, -> (center_id) { where(<<WHERE.strip_heredoc, center_id: center_id) }
    ...
    (item_type LIKE 'Patient' AND item_id IN (SELECT id::varchar FROM patients WHERE patients.center_id = :center_id)) OR
    (item_type LIKE 'Visit' AND item_id IN
      (SELECT id::varchar FROM visits WHERE visits.patient_id IN
        (SELECT id FROM patients WHERE patients.center_id = :center_id))) OR
    (item_type LIKE 'ImageSeries' AND item_id IN
      (SELECT id::varchar FROM image_series WHERE image_series.patient_id IN
        (SELECT id FROM patients WHERE patients.center_id = :center_id))) OR
    (item_type LIKE 'RequiredSeries' AND item_id IN
      (SELECT id::varchar FROM required_series WHERE required_series.visit_id IN
        (SELECT id FROM visits WHERE visits.patient_id IN
          (SELECT id FROM patients WHERE patients.center_id = :center_id)))) OR
    (item_type LIKE 'Image' AND item_id IN
      (SELECT id::varchar FROM images WHERE images.image_series_id IN
...
WHERE
  ...
  scope :for_patient, -> (patient_id) { where(<<WHERE.strip_heredoc, patient_id: patient_id) }
    ...
    (item_type LIKE 'Visit' AND item_id IN
      (SELECT id::varchar FROM visits WHERE visits.patient_id = :patient_id)) OR
    (item_type LIKE 'ImageSeries' AND item_id IN
      (SELECT id::varchar FROM image_series WHERE image_series.patient_id = :patient_id)) OR
    (item_type LIKE 'RequiredSeries' AND item_id IN
      (SELECT id::varchar FROM required_series WHERE required_series.visit_id IN
...
WHERE
  ...
  scope :for_visit, -> (visit_id) { where(<<WHERE.strip_heredoc, visit_id: visit_id) }
    ...
    (item_type LIKE 'ImageSeries' AND item_id IN
      (SELECT id::varchar FROM image_series WHERE image_series.visit_id = :visit_id)) OR
    (item_type LIKE 'RequiredSeries' AND item_id IN
      (SELECT id::varchar FROM required_series WHERE required_series.visit_id = :visit_id)) OR
    (item_type LIKE 'Image' AND item_id IN
      (SELECT id::varchar FROM images WHERE images.image_series_id IN
...
WHERE
  ...
  scope :for_image_series, -> (image_series_id) { where(<<WHERE.strip_heredoc, image_series_id: image_series_id) }
    ...
    (item_type LIKE 'RequiredSeries' AND item_id IN
      (SELECT id::varchar FROM required_series WHERE required_series.image_series_id = :image_series_id)) OR
    (item_type LIKE 'Image' AND item_id IN
      (SELECT id::varchar FROM images WHERE images.image_series_id = :image_series_id))
WHERE
...
WHERE
  scope :for_user, -> (user_id) { where(<<WHERE.strip_heredoc, user_id: user_id) }
    ...
    (item_type LIKE 'UserRole' and item_id IN
      (SELECT id::varchar FROM user_roles WHERE user_roles.user_id = :user_id))
WHERE
