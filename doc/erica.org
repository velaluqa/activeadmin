...
- [[file:erica_store.org][ERICA Store]]
- [[file:erica_remote.org][ERICA Remote]]
- [[file:erica_notifications.org][ERICA Notification Functionality]]
...
Grant all access to this new database to the ~erica~ user:

#+BEGIN_SRC sh
sudo -i -u postgres
psql
> GRANT ALL PRIVILEGES ON DATABASE erica_remote_production to erica;
#+END_SRC
...
*** Use advised huge-page settings
...
Compile and install Redis. For that, follow this guide:
...
Or if the system does not provide the 1.9.3 ruby anymore, install ruby
from source:
...
production:
  adapter: postgresql
  encoding: unicode
  database: erica_remote_production
  pool: 30
  timeout: 5000
  username: erica
  password: [REDACTED]
